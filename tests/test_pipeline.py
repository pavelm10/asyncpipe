    ...
    def define(self) -> List:
        ...
        for idx in range(0, 10):
            ...
            problem_task = TaskTroubleMaker(
                ...
                idx=idx,
                dependency=[ctask],
                fail_softly=self.fail_softly,
            )
            branching_stage_tasks.append(problem_task)

        dtask = TaskD(
            idx=42,
            dependency=branching_stage_tasks,
            propagate_soft_failures=self.propagate_soft_failures,
        )
        return [dtask]
...
def test_pipeline_rerun_with_soft_failures_and_propagating(
    tmp_path, pass_soft_fail
):
